...
export const EvaluasiAdminPage: React.FC = () => {
  ...
  // 2. Fetch Data Buku untuk mencocokkan buku & nomor buku anak
  const { data: bukuList = [] } = useQuery<BukuItem[]>({
    queryKey: ['buku', 'list', 'all'],
    queryFn: async () => {
  ...
  // 3. Fetch Data Evaluasi
  const { data: evaluasiList = [] } = useQuery<EvaluasiItem[]>({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast('Masukan evaluasi berhasil disimpan dan diteruskan ke portal Orang Tua');
    },
    onError: (err: any) => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast('Evaluasi siswa berhasil diperbarui');
    },
    onError: (err: any) => {
  ...
  // Buka Modal Beri Evaluasi untuk Siswa
  const handleOpenEvaluasiModal = (siswa: any, existingEval?: EvaluasiItem) => {
    setTargetSiswa(siswa);
    const prog = siswa.kategori_program?.split(',')[0]?.trim() || 'Sempoa SIP';
    if (existingEval) {
      setEditingEvaluasi(existingEval);
      setFormData({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Main Table */}
      {isSiswaLoading ? (
        <div className="py-16 text-center text-[#757575] text-xs">Memuat data siswa & evaluasi...</div>
      ) : tableData.length === 0 ? (
        <EmptyState
          icon={<EvaluasiIcon size={40} className="text-[#757575]" />}
          title="Belum ada data siswa"
          description="Daftarkan siswa terlebih dahulu di menu Data Siswa."
        />
      ) : (
        <DataTable
          columns={columns}
          data={tableData}
          searchPlaceholder="Cari siswa, UID, nomor buku, catatan evaluasi..."
          searchFilter={(row, q) => {
      ...
      {isModalOpen && targetSiswa && (
        <Modal
          ...
          title={editingEvaluasi ? "Edit Evaluasi Siswa" : "Input Evaluasi & Masukan Siswa"}
          size="md"
        >
          <form
            onSubmit={(e) => {
              ...
              if (editingEvaluasi) {
                updateMutation.mutate({ id: editingEvaluasi.id, data: formData });
              } else {
