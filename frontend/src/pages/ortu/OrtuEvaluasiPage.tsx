...
export const OrtuEvaluasiPage: React.FC = () => {
  ...
  const renderEvaluasiCard = (item: any) => {
      ...
      {evaluasiList.length === 0 ? (
        ...
        <div className="space-y-6">
          {childPrograms.map((p) => {
            ...
            const progEvaluasi = evaluasiList.filter((item) =>
              (item.kategori_program || '').toLowerCase().includes(p.program.toLowerCase()) ||
              p.program.toLowerCase().includes((item.kategori_program || '').toLowerCase())
            );
            return (
              <div key={p.program} className="space-y-3">
                <div className="flex items-center justify-between pb-1 border-b border-[#F1F5F9]">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-black text-[#1E293B]">Lembar Rapor & Evaluasi</h3>
                  </div>
                  <span className="text-[10px] text-[#9E9E9E] font-semibold">{progEvaluasi.length} Lembar</span>
                </div>

                {progEvaluasi.length === 0 ? (
                  <div className="py-6 text-center text-[#94A3B8] text-xs bg-white rounded-2xl border border-[#E0E0E0] p-4 shadow-xs">
                    Belum ada lembar evaluasi untuk program {p.program}.
                  </div>
                ) : (
                  <div className="space-y-3">
                    {progEvaluasi.map((item) => renderEvaluasiCard(item))}
                  </div>
                )}
              </div>
            );
        ...
        <div className="space-y-4">
          {evaluasiList.map((item) => renderEvaluasiCard(item))}
        </div>
      )}
    </div>
  );
