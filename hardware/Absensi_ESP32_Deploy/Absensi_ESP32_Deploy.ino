...
// Gunakan kredensial WiFi / Hotspot yang sesuai di lokasi:
const char* DEFAULT_WIFI_SSID     = "WIFI_SSID_HERE";
const char* DEFAULT_WIFI_PASS     = "WIFI_PASSWORD_HERE";
const char* DEFAULT_ESP32_API_KEY = "ESP32_API_KEY_HERE";
...
bool cekGuruTerlambat(const String& nama, const RtcDateTime& now) {
  ...
  if (nmLower.indexOf("direktur") != -1 || nmLower.indexOf("zulhemawati") != -1) {
    ...
    if (now.DayOfWeek() == 6) return (now.Hour() >= 10);
    return (now.Hour() >= 8);
...
void prosesTap(const RtcDateTime& now) {
  ...
  // Filter Kartu Cacat / Kartu "Magic" Kosong yang belum di-program
  if (uid == "FF FF FF FF" || uid == "00 00 00 00") {
    // Silently ignore: hardware glitch or blank card, no LCD/buzzer changes
    uidTerakhir = uid;
  ...
  if (cachedNama.length() > 0) {
    ...
    bool isLate = cekGuruTerlambat(cachedNama, now);
