...
def save_base64_or_image_to_disk(base64_data_or_bytes: bytes, filename_prefix: str = "galeri") -> str:
    """Helper to save image bytes/base64 to backend/app/uploads/galeri/ with size & format validation"""
    # Max 10MB limit
    if len(base64_data_or_bytes) > 10 * 1024 * 1024:
        raise HTTPException(
    ...
    try:
        ...
        if image.format not in ("JPEG", "JPG", "PNG", "MPO"):
            raise HTTPException(
                ...
                detail="Format file tidak didukung. Harap upload gambar JPG atau PNG."
            )
